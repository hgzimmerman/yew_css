...
pub (crate) fn take_until_encountered<'a>(consumable: &'a str, dont_consume: &'a str) -> impl Fn(&'a str) -> IResult<&'a str, String> {
    let mut last_matched = false;
    debug_assert!(!consumable.chars().any(|x: char| dont_consume.contains(x)));

    move |i: &'a str| {
        let (i, captured) = take_while(|c| !consumable.contains(c) && !dont_consume.contains(c) )(i)?;
